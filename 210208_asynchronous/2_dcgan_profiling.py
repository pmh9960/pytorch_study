...
# ! Today study start at line 600 (Training loop)
...
def run():
    ...
    **Author**: `Nathan Inkawhich <https://github.com/inkawhich>`__
    ...
    dataroot = os.path.join(
        "C:\\Users", "Minho", "Documents", "2021_W", "pytorch_study_data", "celeba"
    )  # ImageFolder
    # dataroot = os.path.join(
    ...
    # * Profiler 사용시에는 multiprocessing이 안됨. => num_workers=0
    with profiler.profile(use_cuda=True) as prof:
        ...
        for epoch in range(num_epochs):
            ...
            for i, data in enumerate(dataloader, 0):
                if i >= 2:
                    break
                with profiler.record_function("Forward D for real"):
                    ############################
                ...
                with profiler.record_function("Backward D for real"):
                ...
                with profiler.record_function("Forward D for fake"):
                    ...
                    # 1/11 수정
                    # 3. detach가 in-place 연산이 아니기 때문에 fake의 CG는 유지가 된다.
                    # 유지를 한뒤 나중에 netG를 학습시킬 때 사용해야 한다. (line 685)
                ...
                with profiler.record_function("Backward D for fake"):
                ...
                with profiler.record_function("Optimize D"):
                    # Update D
                    optimizerD.step()

                with profiler.record_function("Forward G"):
                    ############################
                ...
                with profiler.record_function("Backward G"):
                    # Calculate gradients for G
                    errG.backward()
                    D_G_z2 = output.mean().item()

                with profiler.record_function("Optimize G"):
                    # Update G
                    optimizerG.step()

                with profiler.record_function("Logging"):
    ...
    prof.export_chrome_trace("dcgan.trace")
    exit()
